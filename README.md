...
This is a local data science project built around a medical insurance cost dataset. I wanted to practise a complete, readable workflow: define a target, explore the data, train simple probabilistic classifiers, evaluate them carefully, and explain the mathematics without hiding behind complicated code.
...
The aim is to predict whether a record belongs to the high-cost group using interpretable models:

- Logistic Regression
- Gaussian Naive Bayes

I kept the modelling deliberately simple because the main goal is understanding: probability, odds, Bayes theorem, coefficients, and evaluation metrics.

## Motivation

For an internship portfolio, I wanted a project that shows both mathematical thinking and practical coding. Healthcare data is a good setting for this because it forces careful language: a model can find associations, but that is not the same as proving a medical cause.

One thing I focused on was being able to explain every part of the project in an interview. I avoided advanced models so that the notebook and report stay realistic for a second-year Computer Science and Artificial Intelligence student.
...
```bash
/Users/namem/Documents/Codex/insurance.csv
```
...
The provided file has 1,338 rows and no missing values.
...
- Gaussian Naive Bayes for a simple Bayesian baseline;
...
The `charges` column is not used as a feature because it defines the target. Using it would leak the answer into the model.
...
```bash
cd /Users/namem/Documents/Codex
python3 -m venv .venv
...
The notebook saves figures to `outputs/figures/`, including:
...
In the final train/test split, Logistic Regression and Naive Bayes performed similarly. The exact numbers are in the notebook and CSV table. I would not overstate the result because this is one dataset and one simple split.
...
- Naive Bayes assumes conditional independence, which is probably not fully realistic here.
...
- Write a small model card explaining intended and non-intended use.

## What I Learned

I learned that interpretable models still require careful explanation. Logistic regression is simple to code, but its coefficients only make sense if I understand odds and log-odds. Naive Bayes is also simple to train, but its independence assumption needs to be stated honestly.

I also noticed that smoking status separated the high-cost and not-high-cost groups quite strongly in this dataset. That is an association in the data, not proof of medical causation.
